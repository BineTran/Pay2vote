import { findBINByShortName } from './AccountService';
...
describe('Find BIN by short name', () => {
	it('should return correct BIN when short name is included in name', () => {
		const name = 'Timo plus';
		const code = 'somecode';
		const bin = findBINByShortName(name, code, BankMock);

		expect(bin).toBe('963388');
	});

	it('should return correct BIN when short name is included in name (2)', () => {
		const name = 'Vietinbank iPay';
		const code = 'vietinbank';
		const bin = findBINByShortName(name, code, BankMock);

		expect(bin).toBe('970415');
	});

	it('should return correct BIN when short name is included in name (3)', () => {
		const name = 'Vietinbank iPay';
		const code = 'vietinbank';
		const bin = findBINByShortName(name, code, BankListMock);

		expect(bin).toBe('970415');
	});

	it('should return correct BIN when short name is included in code', () => {
		const name = 'somename';
		const code = 'vietcombank';
		const bin = findBINByShortName(name, code, BankMock);

		expect(bin).toBe('970436');
	});

	it('should return correct BIN when code is included in name', () => {
		const name = 'VCB Digibank';
		const code = 'somecode';
		const bin = findBINByShortName(name, code, BankMock);
